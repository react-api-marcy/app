...
function ChangeView({ center } : { center: [number, number, number?] }) {
  const map = useMap();
...
export default function Map({ location }: { location: UserLocation }) {
  ...
  return (
    <div className="w-[40%] rounded-[25px] overflow-hidden h-full  ">
      <MapContainer center={locationArr} zoom={10} zoomControl={false}>
      <ChangeView center={locationArr} />
        <TileLayer
          attribution=""
          url={darkMode? 'https://tile.jawg.io/b2b117b5-11fc-459b-a4e1-eaf8bc1d91d5/{z}/{x}/{y}{r}.png?access-token=vpxmqZfPKNmXReJ7ArrixBS7jkAMkfhAHIWKG9DZfHAvZzE0M3aIUr3AwXzrMWz9': "https://tile.jawg.io/5f65e2d3-137c-47d6-90e6-3c7bbe8f8bff/{z}/{x}/{y}{r}.png?access-token=vpxmqZfPKNmXReJ7ArrixBS7jkAMkfhAHIWKG9DZfHAvZzE0M3aIUr3AwXzrMWz9"}
        />
        <Marker position={locationArr} icon={customIcon}>
          <Popup>
