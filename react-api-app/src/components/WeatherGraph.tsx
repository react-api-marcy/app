...
import { ForecastResponse, getHourlyWeatherStats } from "../utils";
...
function WeatherGraph({values}: {values: ForecastResponse}) {
  const data = getHourlyWeatherStats(values)
  console.log(data)
  return (
    <div className="w-[61.5%]  pb-[5rem]  bg-opacity-15 bg-white  rounded-[25px]">
      ...
      <AreaChart data={data}    margin={{ top: 50, right: 0, left: 0, bottom: 0 }}>
        ...
        <Tooltip />
        <Area type="monotone" dataKey="temp" stroke="rgba(130, 202, 157, 0.2)" fillOpacity={1} fill="url(#tempColor)">
          <LabelList   dataKey="temp" content={renderCustomizedLabel} position="top" style={{ fill: 'white' }} />
        </Area>
