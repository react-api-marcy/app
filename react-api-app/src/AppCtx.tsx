...
function getSettingsItem<T>(item: string, def: T) {
  return () => {
    try {
...
export default function AppCtxProvider({ children }: { children: JSX.Element | JSX.Element[] }) {
  ...
  // Default to dark mode between 6 AM and 7 PM if the user hasn't selected a preference
  const def = currentHour >= 19 || currentHour < 6;
